...
import { CaptionsResponse, TranscriptSegment, TranscriptData } from "@/types/video";
...
import { AlertCircle, FileText, Clock, Languages, Copy, Check } from "lucide-react";
...
export default function CaptionsPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const msg = e.status === 401 ? "Authorization required. Please authenticate with Google." :
                  e.status === 404 ? "No captions available for this video" :
  ...
  const getSegments = (data: TranscriptData | undefined): TranscriptSegment[] => {
    if (!data) return [];
  ...
  const currentTranscripts = getSegments(captionData?.transcripts[selectedLang]);
            ...
            <Card className="border-0 rounded-2xl bg-card overflow-hidden">
              <div className="flex flex-col md:flex-row">
                <div className="md:w-80 flex-shrink-0">
                  <img 
                    src={captionData.videoInfo.thumbnailUrl.maxresdefault || captionData.videoInfo.thumbnailUrl.hqdefault} 
                    alt={captionData.videoInfo.name}
                ...
                <CardContent className="flex-1 p-5">
                  ...
                  <div className="flex flex-wrap gap-3 text-sm text-muted-foreground">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {captionData.language_code.length} language(s)
                    </span>
                  </div>
              ...
              <Card className="border-0 rounded-2xl bg-card">
                ...
                <CardContent>
                  {captionData.language_code.length > 1 && (
                    <Tabs value={selectedLang} onValueChange={setSelectedLang} className="mb-4">
